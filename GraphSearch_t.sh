...
public:
     ...
     * @return  匹配 h 的子图列表。
     */
    void findOrderedSubgraphs(LockFreeQueue<SearchData> *task_queue, int *edge_idx, std::mutex *edge_mutex, int thread_id, int thread_num);

    /**
     * 处理新任务的查找
     * @param edge_idx  块起始边索引
     */
    void findNewOrderedSubgraphs(int *edge_idx, std::mutex *edge_mtx, int thread_id, int thread_num, std::atomic<int> *phase_tasks_beg, std::atomic<int> *phase_tasks_end);

    /**
     * 线程对于窃取到的任务进行处理
     */
    void stealOrderedSubgraphs(int *edge_idx, std::mutex *edge_mtx, int thread_id, int thread_num, std::atomic<int> *phase_tasks_beg, std::atomic<int> *phase_tasks_end);
...
public:
     ...
     */
    int findStart(int g_i, const std::vector<int> &edgeIndexes, std::vector<int> &all_edge, BinaryRange *binary_range, int bg, int ed);

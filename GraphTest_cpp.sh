 ...
 * 修改部分内容即可
 */
void GraphSearch::findOrderedSubgraphs(LockFreeQueue<SearchData> *task_queue, int *edge_idx, std::mutex *edge_mutex, int thread_id, int thread_num)
{
    ...
    while (idx < ed)
    {
...

        {
            ...
            this->setData(std::move(task_queue->at(idx)), false);
        ...
        while (true)
        {
            ...
            if (candidates[h_i].first > candidates[h_i].second || candidates[h_i].first >= arr[h_i]->size())
            {
            ...
            for (idx = candidates[h_i].first; idx <= candidates[h_i].second; idx++)
            {
                ...
                if (cur_time >= p->second)
                {
                    ...
                    q->at(id).enqueue(std::move(this->getData()));
...
void GraphSearch::findNewOrderedSubgraphs(int *edge_idx, std::mutex *edge_mtx, int thread_id, int thread_num, std::atomic<int> *phase_tasks_beg, std::atomic<int> *phase_tasks_end)
{
    /**
     * 只有一开始的时候需要初始化
     * 由于重复利用变量，需要初始化
    ...
    while (true)
    {
...

        {
            ...
            while (true)
            {
                ...
                if (pool->steal_flag[thread_id].compare_exchange_strong(excepted, true, std::memory_order_acquire, std::memory_order_relaxed))
                {
                     ...
                     * 一次性获取nums个任务
                     */
                    start_idx = phase_tasks_beg[thread_id].fetch_add(nums, std::memory_order_release);
                    end_idx = std::min(int(phase_tasks_end[thread_id].load(std::memory_order_relaxed)), start_idx + nums);
        ...
        for (int start = start_idx; start < end_idx; start++)
        {
            g_i = start;
            // }
            ...
            while (true)
            {
                ...
                for (idx = candidates[h_i].first; idx <= candidates[h_i].second; idx++)
                {
                     ...
                     */
                    // if (times >= 30 && pool->group_finish[pool->group_size * part_idx + thread_id / 4].load(std::memory_order_acquire) > 0)
                    // {
                    //     int id = thread_id / 4;
                    //     bool flag = false;
                    ...
                    //         bool excepted = true;
                    //         /**
                    //          * 确保一个空闲线程只可以被一个运行线程分配任务
                    //          */
                    //         if (pool->re_distribute[i].compare_exchange_strong(excepted, false, std::memory_order_acquire, std::memory_order_relaxed))
                    //         {
                    //             /**
                    //              * 获取上下文数据
                    //              */
                    //             // pool->vec_graph[i].setData(std::move(this->getData()));
                    //             {
                    //                 pool->vec_graph[i]._firstEdgeTime = _firstEdgeTime;
                    //                 // pool->vec_graph[i]._h2gNodes = _h2gNodes;
                    //                 // pool->vec_graph[i]._sg_edgeStack = _sg_edgeStack;
                    //                 // pool->vec_graph[i].candidates = candidates;

                    //                 for(int j = 0; j < 5; j++)
                    //                 {
                    //                     pool->vec_graph[i]._h2gNodes[j] = _h2gNodes[j];
                    //                     pool->vec_graph[i].candidates[j] = candidates[j];
                    //                 }

                    //                 pool->vec_graph[i].h_i = h_i;
                    //                 // data.g_i = g_i;
                    //                 // pool->vec_graph[i].map = map;
                    //                 pool->vec_graph[i].arr[0] = arr[0];
                    //                 pool->vec_graph[i].arr[1] = arr[1];
                    //                 pool->vec_graph[i].arr[2] = arr[2];
                    //                 pool->vec_graph[i].arr[3] = arr[3];
                    //                 pool->vec_graph[i].arr[4] = arr[4];
                    //             }
                    //             // std::cout << "re_distribute " << i << std::endl;
                    //             // std::cout << "re_distribute " << i << std::endl;
                    //             for (int j = 1; j <= h_i; j++)
                    //             {
                    //                 /**
                    ...
                    //                  */
                    //                 candidates[j].first++;
                    //                 pool->vec_graph[i].candidates[j].second = min(pool->vec_graph[i].candidates[j].first, pool->vec_graph[i].candidates[j].second);
                    //             }
                    //             // 启动让它去执行
                    //             // pool->re_distribute[i] == false;
                    //             pool->distribute[i].store(true, std::memory_order_release);
                    //             flag = true;
                    //         }
                    //     }
                    //     // 回归状态
 ...
 */
void GraphSearch::stealOrderedSubgraphs(int *edge_idx, std::mutex *edge_mtx, int thread_id, int thread_num, std::atomic<int> *phase_tasks_beg, std::atomic<int> *phase_tasks_end)
{
    /**
     * 直接继承分配的任务数据
     */
 ...
 * 找不到，则返回列表大小
 */
int GraphSearch::findStart(int g_i, const std::vector<int> &edgeIndexes, std::vector<int> &all_edge, BinaryRange *binary_range, int bg, int ed)
{
    // 对应于匹配起始边，即所有边都作为起始边
    /**
    ...
    if (edgeIndexes.back() < g_i)
        return edgeIndexes.size();
    if (edgeIndexes.front() >= g_i && _g->_edges[edgeIndexes.front()].time() <= last_time)
        return 0;
...
int GraphSearch::findEnd(int g_i, const std::vector<int> &edgeIndexes, Graph &g, BinaryRange *binary_range, int bg, int ed)
{
    ...
    while (left < right)
    {
        ...
        // cache缺失较严重1.56%
        if (g._edges[mid_idx].time() <= last_time)
        {
            left = mid;
            /**
...
void GraphSearch::findNextMatch(int h_i, int g_i, std::vector<int> *all_edge_idxs, Graph &g)
{
    ...
    if (minfo.io == 1)
    {
        ...
        // if (part_idx > out_range[base].second)
        // {
        //     bg = searchEdges->size();
        // }
        // else
        // {
        //     /**
        //      * out_part_row的base索引表示当前块的起始位置，part_ix - out_part_col_idx[当前起始元素索引]
        //      * 获取具体元素所处的位置，就可以直接通过元素数组获取
        //      */
        //     int t = out_part_row_ptr -> at(base);
        //     int idx = t + part_idx - out_part_col_idx -> at(t);
        //     bg = out_part_values -> at(idx);
        // }

        // if (part_idx == p_s -> size() - 1 || part_idx >= out_range[base].second)
        // {
        //     /**
        //      * 空边情况直接-1；无解情况也只是无解
        //     */
        ...
        // {
        //     int t = out_part_row_ptr -> at(base);
        //     int idx = t + part_idx - out_part_col_idx -> at(t) + 1;
        //     ed = out_part_values -> at(idx);
        // }
    }
    else if (minfo.io == 0)
    {
        ...
        // if (part_idx > in_range[base].second)
        // {
        //     bg = searchEdges->size();
        // }
        // else
        // {
        //     int t = in_part_row_ptr -> at(base);
        //     int idx = t + part_idx - in_part_col_idx -> at(t);
        //     bg = in_part_values -> at(idx);
        // }

        // if (part_idx == p_s->size() - 1 || part_idx >= in_range[base].second)
        // {
        //     ed = searchEdges->size() - 1;
        // }
        // else
        // {
        //     // ed = in_binary_range[base * p_s->size() + part_idx + 1].this_block_start;
        //     int t = in_part_row_ptr -> at(base);
        //     int idx = t + part_idx - in_part_col_idx -> at(t) + 1;
        //     ed = in_part_values -> at(idx);
        // }
    }
    else
    {
        ...
        // std::cout << h_i << " " << minfo.baseNode << " " << minfo.constraintNode << std::endl;
        // for(int i = 0; i < 5; i++)
        // {
        //     std::cout << i << ": " << _h2gNodes[i] << std::endl;
        // }
        ...
        if (uEdges.size() < vEdges.size())
        {
            ...
            // if (part_idx > out_range[base1].second)
            // {
            //     bg = searchEdges->size();
            // }
            // else
            // {
            //     /**
            //      * out_part_row的base索引表示当前块的起始位置，part_ix - out_part_col_idx[base]表示偏移位置
            //      * 获取具体元素所处的位置，就可以直接通过元素数组获取
            //      */
            //     int t = out_part_row_ptr->at(base1);
            //     int idx = t + part_idx - out_part_col_idx->at(t);
            //     bg = out_part_values->at(idx);
            // }

            // if (part_idx == p_s->size() - 1 || part_idx >= out_range[base1].second)
            // {
            //     ed = searchEdges->size() - 1;
            // }
            // else
            ...
            // 确定一个节点为base1，则需要确保另一个节点一定为dest，即为baseNode对应的映射
        ...
        else
        {
            ...
            // if (part_idx > in_range[base0].second)
            // {
            //     bg = searchEdges->size();
            // }
            // else
            // {
            //     int t = in_part_row_ptr->at(base0);
            //     int idx = t + part_idx - in_part_col_idx->at(t);
            //     bg = in_part_values->at(idx);
            // }

            // if (part_idx == p_s->size() - 1 || part_idx >= in_range[base0].second)
            // {
            //     ed = searchEdges->size() - 1;
            // }
            // else
    ...
    // 如果查询图的两个节点已经被映射，则仅搜索这些节点的关联边（优化性能）
    ...
    //     {
    //         searchEdges = &uEdges; // 使用较小的边集合
    //         binary_range = &out_binary_range->at(_h2gNodes[h_u])[part_idx];
    //     }
    //     else
    //     {
    //         searchEdges = &vEdges;
    //         binary_range = &in_binary_range->at(_h2gNodes[h_v])[part_idx];
    //     }
    // }
    // else if (_h2gNodes[h_u] >= 0) // 如果只有起点被映射
    // {
    //     searchEdges = &_g->nodes()[_h2gNodes[h_u]].outEdges();
    //     binary_range = &out_binary_range->at(_h2gNodes[h_u])[part_idx];
    // }
    // else if (_h2gNodes[h_v] >= 0) // 如果只有终点被映射
    // {
    ...
    if (candidates[h_i].first == -1)
    {
        beg = findStart(g_i, *searchEdges, *all_edge_idxs, binary_range, bg, ed);
...
bool GraphSearch::findNextMatch(int h_i, int g_idx)
{
    ...
    // 获取原始图中的当前边
    const Edge &g_edge = _g->_edges[g_idx];
	...
	if (minfo -> newNode == 1)
	{
		...
		{
		case 5:
			checked = checked && (_h2gNodes[4] != node);
		case 4:
			checked = checked && (_h2gNodes[3] != node);
		case 3:
			checked = checked && (_h2gNodes[2] != node);
		case 2:
			checked = checked && (_h2gNodes[0] != node) && (_h2gNodes[1] != node);
		}
	}
	...
	else if(minfo -> newNode == 2)
	{
		...
		{
		case 5:
			checked = checked && (_h2gNodes[4] != node0);
		case 4:
			checked = checked && (_h2gNodes[3] != node0);
		case 3:
			checked = checked && (_h2gNodes[2] != node0);
		case 2:
			checked = checked && (_h2gNodes[0] != node0) && (_h2gNodes[1] != node0);
		}
		if(node0 != node1);
		{
			switch (minfo -> mappedNodes)
			{
			case 5:
				checked = checked && (_h2gNodes[4] != node1);
			case 4:
				checked = checked && (_h2gNodes[3] != node1);
			case 3:
				checked = checked && (_h2gNodes[2] != node1);
			case 2:
				checked = checked && (_h2gNodes[0] != node1) && (_h2gNodes[1] != node1);
			}
		}
	}
	return checked;
